...
def svm_loss_vectorized(
    ...
    num_classes = W.shape[1]
    num_train = X.shape[0]
    scores = X.mm(W)  # shape [N, D] x [D, C] = [N, C]
    # print(scores.shape)
    correct_class_score = scores[range(num_train), y].reshape((-1, 1))
    margin = scores - correct_class_score + 1
    margin[range(num_train), y] = 0  # j != y[i]
    margin[margin <= 0] = 0
    ...
    #############################################################################
    # Replace "pass" statement with your code
    # margin 在循环实现的部分其实就可以视为一个[N, C]的矩阵，1代表此时的损失大于0
    # 从列的角度看，每当一个样本中某一个不正确类别预测的分数比较大，当前样本对应的不正确类别的梯度增加，正确类别的梯度减少
    X_factor = (margin > 0).double()
    # print(X_factor)
    X_factor[range(num_train), y] -= torch.sum(X_factor, dim=1)
    # print(X_factor)
    dW = torch.mm(X.T, X_factor)
    dW /= num_train
    dW += 2 * reg * W
    #############################################################################
    #                             END OF YOUR CODE                              #
...
def sample_batch(
    ...
    #########################################################################
    # Replace "pass" statement with your code
    indices = torch.randint(low=0, high=X.shape[0], size=(X.shape[0],))
...
def svm_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [1e-2, 9e-3, 1.5e-3, 2.5e-2, 2e-2]
    regularization_strengths = [7e-3, 5e-2, 8e-3, 5e-3, 6e-3]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def test_one_param_set(
    ...
    train_acc = (y_train_pred == data_dict['y_train']).sum() / data_dict['X_train'].shape[0]

    y_val_pred = cls.predict(data_dict['X_val'])
    val_acc = (y_val_pred == data_dict['y_val']).sum() / data_dict['X_val'].shape[0]
    ############################################################################
    #                            END OF YOUR CODE                              #
    ############################################################################
...
def softmax_loss_naive(
    ...
    num_classes = W.shape[1]
    num_train = X.shape[0]
    scores = torch.exp(X.mm(W))
    for i in range(num_train):
        loss += - torch.log(scores[i][y[i]] / torch.sum(scores[i]))
    ...
    for i in range(num_train):
        for j in range(num_classes):
            dW[:, j] += scores[i][j] * X[i] / torch.sum(scores[i])
            if j == y[i]:
                # if this is a true label
                dW[:, j] -= X[i]
    dW /= num_train
    dW += 2 * reg * W
    #############################################################################
...
def softmax_loss_vectorized(
    ...
    num_train = X.shape[0]
    scores = torch.exp(X.mm(W))
    sum_scores = torch.sum(scores, dim=1, keepdim=True)
    prob = scores / sum_scores
    loss = torch.sum(- torch.log(prob[range(num_train), y]))
    loss /= num_train
    loss += reg * torch.sum(W * W)
    ...
    # W: [D, C]
    # X_factor: [N, C]
    X_factor = scores / sum_scores
    X_factor[range(num_train), y] -= 1.0
    dW = X.T.mm(X_factor)
    dW /= num_train
    dW += 2 * reg * W
    #############################################################################
