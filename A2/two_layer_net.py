...
def nn_forward_backward(
    ...
    ############################################################################
    # Replace "pass" statement with your code
    num_hidden = scores.shape[0]
    num_class = scores.shape[1]
    scores = torch.exp(scores)
    sum_scores = torch.sum(scores, dim=1, keepdim=True)
    prob = scores / sum_scores
    # print(prob.shape)
    loss = torch.sum(- torch.log(prob[range(num_hidden), y]))  # softmax loss
    loss /= num_hidden
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    # hint: a computation graph really helps!
    dLdy2 = prob  # [N, C]
    dLdy2[range(N), y] -= 1.0  # correct class
    dLdy2 /= N
    dLdb2 = torch.sum(dLdy2, dim=0)  # [C]
    grads['b2'] = dLdb2
    dLdw2 = h1.t().mm(dLdy2)  # [H, N] * [N, C] = [H, C]
    grads['W2'] = dLdw2 + 2 * reg * W2
    dLdh1 = dLdy2.mm(W2.t())  # [N, C] * [C, H] = [N, H]
    y1 = X.mm(W1) + b1  # [N, D] * [D, H] = [N, H]
    dh1dy1 = y1  # [N, H]
    dh1dy1[dh1dy1 <= 0] = 0
    dh1dy1[dh1dy1 > 0] = 1
    dLdy1 = dLdh1 * dh1dy1  # [N, H]
    dLdw1 = X.t().mm(dLdy1)  # [D, N] * [N, H] = [D, H]
    dLdb1 = torch.sum(dLdy1, dim=0)  # [C]
...
def nn_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [1e-4, 1e-3, 1e-2, 1e-1, 5e-1, 1e0]
    hidden_sizes = [64, 128, 256]
    regularization_strengths = [1e-4, 5e-4, 1e-5, 5e-5, 5e-3]
