...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_test_vec = x_test.reshape(num_test, -1)
    for i in range(num_train):
        for j in range(num_test):
            dists[i][j] = torch.sqrt(torch.sum(torch.pow(x_train_vec[i] - x_test_vec[j], 2)))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_test_vec = x_test.reshape(num_test, -1)
    for i in range(num_train):
        dists[i, :] = torch.sqrt(torch.sum(torch.pow(x_train_vec[i] - x_test_vec, 2), dim=1))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # Answer:
    #   unfold the formula of Euclidean distance: (x - y)^2 = x^2 + y^2 - 2xy
    dists = torch.sqrt(
        torch.sum(torch.pow(x_train_vec, 2), dim=1).unsqueeze(1) +
        torch.sum(torch.pow(x_test_vec, 2), dim=1).unsqueeze(0) -
        2 * torch.mm(x_train_vec, x_test_vec.t()))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    values, indices = torch.topk(input=dists, dim=0, k=k, largest=False)
    # print(values)
    # print(indices)
    labels = torch.zeros(size=(num_test, k))
    for i in range(num_test):
        labels[i] = y_train[indices[:, i]]
    # print(labels)
    for i in range(labels.shape[0]):
        count = torch.zeros(num_test)
        pred = 0
        pred_times = 0
        for j in range(labels.shape[1]):
            label = int(labels[i, j])
            count[label] += 1
            if count[label] > pred_times:
                pred_times = count[label]
                pred = label
            elif count[label] == pred_times and label < pred:
                pred = label
        y_pred[i] = torch.tensor(pred)
...
def knn_cross_validate(
    ...
    for k in k_choices:
        ...
        for index in range(num_folds-1):
            classifier = KnnClassifier(x_train_folds[index], y_train_folds[index])
            accuracy = classifier.check_accuracy(x_train_folds[num_folds-1], y_train_folds[num_folds-1], k=k)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k in k_to_accuracies.keys():
        acc_mean = torch.mean(torch.tensor(k_to_accuracies[k]))
        if acc_mean > max_mean:
            max_mean = acc_mean
            best_k = k
    ##########################################################################
    #                           END OF YOUR CODE                             #
